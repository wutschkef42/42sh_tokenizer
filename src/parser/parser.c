...
int	parse_command(t_list **tokens, t_ast_node **ast)
{
	...
	while (match((t_tok*)(*tokens)->data, TYPE_word) || match((t_tok*)(*tokens)->data, TYPE_less ||
			match((t_tok*)(*tokens)->data, TYPE_dless) || match((t_tok*)(*tokens)->data, TYPE_great)))
	{
		ft_lstadd(&cmd_tokens, *tokens);
		next_token(tokens);
	}
